...
<html >
    ...
    <body>
        ...
        <div class="product-view">
            <form action="productViewCategory" method="get" id="categoryForm">
                <label for="categoryId">Select Category:</label>
                <select name="categoryId" id="categoryId" onchange="submitForm()">
                    <option value="">All Categories</option>
                    <%
                        // Loop through the categories to create options
            ...
            <% 
            // Check if categories and products are not null
            if (categories != null && products != null) {
                for (Category category : categories) {
                    boolean hasProduct = false;
            %>
            <h2 class="category-title" id="category-<%= category.getCategoryId() %>"><%= category.getCategoryName() %></h2>
            <ul class="product-list">
                <%
                        // Loop through products and match categoryId
                        for (Product product : products) {
                            if (product.getCategoryId() == category.getCategoryId()) {
                                hasProduct = true;
                %>
                <li class="product-item" id="<%= product.getProductId() %>">
                    <img src="<%= product.getSourceImg() %>" alt="<%= product.getProductName() %>" class="product-image">
                    ...
                    <form action="addCartServlet" method="post">
                        ...
                        <input type="hidden" name="stockQuantity" value="<%= product.getStockQuantity() %>">
                        <input type="hidden" name="categoryId" value="<%= product.getCategoryId() %>">
                        <input type="hidden" name="sourceImg" value="<%= product.getSourceImg() %>">
                ...
                <%
                        ...
                        if (!hasProduct) {
                %>
                <li class="no-products">No products available for this category</li>
                    <%
                            }
                    %>
            </ul>
